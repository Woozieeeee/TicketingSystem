...
import { ArrowLeft, User as UserIcon, Calendar, Clock, TrendingUp, ShieldCheck, Download } from "lucide-react";
...
const IT_TEAM = [
  ...
  { id: "u4", name: "Palku Chupapi", role: "Helpdesk", trend: [40, 80, 60, 120, 100, 180, 210], color: "#8b5cf6" },
  { id: "u5", name: "Christine Rose", role: "Systems Engineer", trend: [15, 30, 25, 50, 45, 70, 95], color: "#ec4899" },
  { id: "u6", name: "Matt", role: "Database Admin", trend: [10, 25, 20, 45, 35, 60, 82], color: "#06b6d4" },
];
    ...
    <div className="p-6 md:p-10 bg-[#f8fafc] min-h-screen text-slate-900 font-sans relative">
      ...
      <header className="mb-12 flex items-center justify-between gap-8">
        <div className="flex items-center gap-6 shrink-0">
          {view === "stats" && (
            <button onClick={() => setView("list")} className="p-3 bg-white border border-slate-200 rounded-2xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
              <ArrowLeft size={20} />
            </button>
          )}
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 block mb-1">IT Head View</span>
            <h1 className="text-4xl font-black tracking-tighter leading-none uppercase">Monitoring</h1>
          </div>
        </div>
      ...
      {view === "stats" && (
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <MetricCard label="Pending" value={currentStats.pending} sub={`Records for ${displayDate}`} border="border-slate-200" />
            <MetricCard label="Ongoing" value={currentStats.ongoing} sub={`Active on ${displayDate}`} border="border-amber-200" />
            <MetricCard label="Resolved" value={currentStats.resolved} sub={`Closed on ${displayDate}`} border="border-emerald-200" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white border border-slate-100 p-10 rounded-[48px] shadow-sm">
               <div className="flex items-center justify-between mb-12">
                  <h3 className="text-2xl font-black tracking-tight">{timeRange} Analytics: {displayDate}</h3>
                  <div className="flex bg-slate-50 border border-slate-200 p-1 rounded-xl">
                    {["Today", "Weekly", "Monthly"].map(r => (
                      <button key={r} onClick={() => setTimeRange(r)} className={`px-4 py-2 rounded-lg text-[10px] font-bold transition-all ${timeRange === r ? "bg-white shadow-sm text-slate-900" : "text-slate-400"}`}>
                        {r.toUpperCase()}
                      </button>
                    ))}
                  </div>
            ...
            <div className="bg-white border border-slate-100 p-10 rounded-[48px] flex flex-col justify-between shadow-xl relative overflow-hidden group">
               ...
               <div className="relative z-10">
                  <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-full mb-10">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Security Verified</span>
                  </div>
                  
                  <h4 className="text-4xl font-black italic tracking-tighter mb-4 leading-tight">
                    Stats for <span className="text-emerald-600">{selectedUser.name}</span>
                  </h4>
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest leading-loose">
                    Automated daily backup confirmed. Performance is within target margins for the selected period.
                  </p>
               </div>
...
function MetricCard({ label, value, sub, border }: any) {
  return (
    <div className={`p-10 bg-white border ${border} rounded-[40px] shadow-sm`}>
      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-4 italic">{label}</span>
      <span className="text-7xl font-black tracking-tighter leading-none">{value}</span>
      <p className="mt-6 text-[9px] font-bold text-slate-300 uppercase tracking-widest">{sub}</p>
    </div>
  );
}